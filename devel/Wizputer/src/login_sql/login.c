...
//------------------------------
#define EMBEDDED // For testing

#ifdef EMBEDDED

static char *embedded_options[] = { "login", "--no-defaults", "--basedir=./mysql_data", "--datadir=login" };
int num_options = sizeof(embedded_options)/ sizeof(char *);

static char *embedded_groups[] = { "server", "embedded", NULL };
...
int mmo_auth_sqldb_init(void) {
	...
	if(!mysql_handle) {
		ShowMessage("MySQL Init Failed!");
		exit(1);
	}
	
	ShowMessage("Connect to Login MySQL Server...\n");

#ifdef EMBEDDED
	mysql_options(mysql_handle, MYSQL_OPT_USE_EMBEDDED_CONNECTION, (char* )NULL);
	ShowMessage("Using Embedded Connection.\n");

	// DB connection start
	if (!mysql_real_connect(mysql_handle, NULL, NULL, NULL, "ragnarok", 0, (char *)NULL, 0)) {
#else
...
void mmo_db_close(void) {
	...
	mysql_server_end();
#endif
	ShowMessage("MySQL Connection Closed...\n");
		...
		else
		{
			...
			{	//ok no existing acc,
				ShowMessage("Adding a new account user: %s with passwd: %s sex: %c (ip: %s)\n", account->userid, account->passwd, account->userid[len+1], ip_str);
							sprintf(tmpsql, "INSERT INTO `%s` (`%s`, `%s`, `sex`, `email`) VALUES ('%s', '%s', '%c', '%s')", login_db, login_db_userid, login_db_user_pass, account->userid, account->passwd, account->userid[len+1], "a@a.com");
				if(mysql_SendQuery(mysql_handle, tmpsql))
		...
		case 0x2712:
		  {
			...
			if (i != AUTH_FIFO_SIZE) { // send account_reg
				...
				unsigned char level;
				account_id=RFIFOL(fd,2);
				sprintf(tmpsql, "SELECT `email`,`connect_until`,`%s` FROM `%s` WHERE `%s`='%ld'", login_db_level, login_db, login_db_account_id, account_id);
				if (mysql_SendQuery(mysql_handle, tmpsql)) {
					ShowMessage("DB server Error - %s\n", mysql_error(mysql_handle));
				}
				sql_res = mysql_store_result(mysql_handle) ;
...
int do_init(int argc,char **argv){
	...
	//Auth start
	ShowMessage ("Initializing MySQL...\n");
#ifdef EMBEDDED
	if(mysql_server_init(num_options, embedded_options, (char **)embedded_groups)) {
		ShowMessage( "Failed to initialize embedded MySQL server!\n");
		exit(1);
	}
	ShowMessage ("Embedded MySQL initialized...\n");
#endif
	mmo_auth_sqldb_init();
	ShowMessage ("MySQL Initialized...\n");
