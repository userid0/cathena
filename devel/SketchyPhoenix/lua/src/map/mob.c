...
int mob_parse_dataset(struct spawn_data *data)
{
	...
	//Check the size of the event length.
	if ((strlen(data->function)) >= EVENT_NAME_LENGTH)

	if( npc_event_isspecial(data->function) )
	{//Portable monster big/small implementation. [Skotlex]
		int i = atoi(data->function);
		...
		data->function[0] = '\0'; //Clear event as it is not used.
	}
	else if( ( len = strlen(data->function) ) > 0 )
	{
		if( data->function[len-1] == '"' )
			data->function[len-1] = '\0'; //Remove trailing quote.
		if( data->function[0] == '"' ) //Strip leading quotes
