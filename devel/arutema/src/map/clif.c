...
/*						End Addition [Valaris]			*/
		...
		for(i=0;i < SC_SENDMAX; i++)
			if(sd->sc_data[i].timer!=-1 && (battle_config.display_hallucination || i != SC_HALLUCINATION))
...
void clif_parse_GlobalMessage(int fd, struct map_session_data *sd) { // S 008c <len>.w <str>.?B
	...
	if (berserk) {
		switch(rand() % 3)
		{
			case 1: sprintf(buf, "Eeeeaaagggghhhhh!");
			case 2: sprintf(buf, "Death!");
			default: sprintf(buf, "Banzai!");
		}
	}
	else buf = (unsigned char*)aCallocA(RFIFOW(fd,2) + 4, sizeof(char));

	message = (char*)aCallocA(RFIFOW(fd,2) + 128, sizeof(char));


	//printf("clif_parse_GlobalMessage: message: '%s'.\n", RFIFOP(fd,4));
	if (!berserk && strncmp((char*)RFIFOP(fd,4), sd->status.name, strlen(sd->status.name)) != 0) {
