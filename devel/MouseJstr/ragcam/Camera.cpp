...
{	
	...
	if(!(GetKeyState(VK_SHIFT) & 0x80) && (GetKeyState(VK_UP) & 0x80 || GetKeyState('W') & 0x80)) {				
	...
	if(!(GetKeyState(VK_SHIFT) & 0x80) && (GetKeyState(VK_DOWN) & 0x80 || GetKeyState('S') & 0x80)) {			
	...
	if(GetKeyState(VK_SHIFT) & 0x80 && GetKeyState('W') & 0x80) {			
		DollyCamera(-speed);
	}	

	if(GetKeyState(VK_SHIFT) & 0x80 && GetKeyState('S') & 0x80) {			
		DollyCamera(speed);
	}	
