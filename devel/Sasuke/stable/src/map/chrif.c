...
int chrif_save(struct map_session_data *sd)
{
	...
	if( !session_isActive(char_fd) || !chrif_isconnect() ) //Shino's Session checks [Sasuke]
		return -1;
...
int chrif_connect(int fd)
{
	...
	if( !session_isActive(fd) ) //Shino's Session checks [Sasuke]
		return -1;
...
int chrif_recvmap(int fd)
{
	...
	if( !session_isActive(char_fd) || !chrif_isconnect() )	// まだ準備中 //Shino's Session checks [Sasuke]
		return -1;
...
int chrif_removemap(int fd)
{
	...
	if( !session_isActive(fd) || !chrif_isconnect() ) //Shino's Session checks [Sasuke]
		return -1;
...
int chrif_parse(int fd)
{
	...
	if (fd != char_fd && session_isValid(fd))
	{
		close(fd);
		delete_session(fd);
		return 0;
...
int send_users_tochar(int tid, unsigned int tick, int id, int data)
{
	...
	if( !session_isActive(char_fd) || !chrif_isconnect() ) // Thanks to Toster //Shino's Session check [Sasuke]
		return 0;
