...
in_addr_t login_ip=INADDR_ANY;	// bind on all addresses by default
		...
		default:
			{
				...
				if (logfp) {
					...
					fprintf(logfp, "parse_fromchar: connection #%d (ip: %s), packet: 0x%x (with being read: %d)." RETCODE, fd, ip, (unsigned short)RFIFOW(fd,0), RFIFOREST(fd));
		...
		default:
			{
				...
				if (logfp) {
					...
					strftime(tmpstr, 23, date_format, localtime((const time_t*)&(tv.tv_sec)));
					fprintf(logfp, "%s.%03d: receiving of an unknown packet -> disconnection" RETCODE, tmpstr, (int)tv.tv_usec / 1000);
					fprintf(logfp, "parse_admin: connection #%d (ip: %s), packet: 0x%x (with being read: %d)." RETCODE, fd, ip, (unsigned short)RFIFOW(fd,0), RFIFOREST(fd));
					fprintf(logfp, "Detail (in hex):" RETCODE);
					fprintf(logfp, "---- 00-01-02-03-04-05-06-07  08-09-0A-0B-0C-0D-0E-0F" RETCODE);
					memset(tmpstr, '\0', sizeof(tmpstr));
					for(i = 0; i < RFIFOREST(fd); i++) {
...
//	       subnetmaski[0], subnetmaski[1], subnetmaski[2], subnetmaski[3]);
	...
	ShowMessage("LAN test (result): %s source.\n"CL_RESET, (lancheck) ? CL_BT_CYAN"LAN" : CL_BT_GREEN"WAN");
	return lancheck;
}
			...
			if (result == -1) {
				...
				if (min_level_to_connect > gm_level) {
					...
					for(i = 0; i < MAX_SERVERS; i++) {
						if (server_fd[i] >= 0) {
							if (lan_ip_check(p))
								WFIFOLIP(fd,47+server_num*32) = inet_addr(lan_char_ip);
							else
		...
		default:
			if (save_unknown_packets) {
				...
				if (logfp) {
					...
					strftime(tmpstr, 23, date_format, localtime((const time_t*)&(tv.tv_sec)));
					fprintf(logfp, "%s.%03d: receiving of an unknown packet -> disconnection" RETCODE, tmpstr, (int)tv.tv_usec / 1000);
					fprintf(logfp, "parse_login: connection #%d (ip: %s), packet: 0x%x (with being read: %d)." RETCODE, fd, ip, (unsigned short)RFIFOW(fd,0), RFIFOREST(fd));
					fprintf(logfp, "Detail (in hex):" RETCODE);
					fprintf(logfp, "---- 00-01-02-03-04-05-06-07  08-09-0A-0B-0C-0D-0E-0F" RETCODE);
					memset(tmpstr, '\0', sizeof(tmpstr));
					for(i = 0; i < RFIFOREST(fd); i++) {
...
int parse_console(char *buf) {
	...
	if(strcasecmp("shutdown", command) == 0 ||
		...
		ShowMessage(CL_LT_CYAN"Console: "CL_NORM""CL_BOLD"I'm Alive.\n"CL_RESET);
	...
	while(fgets(line, sizeof(line)-1, fp)) {
		...
		if (strcasecmp(w1, "lan_char_ip") == 0) { // Read Char-Server Lan IP Address
			...
			if (h != NULL) {
				...
				memcpy(lan_char_ip, w2, sizeof(lan_char_ip));
				lan_char_ip[sizeof(lan_char_ip)-1] = '\0';
			}
			ShowMessage("LAN IP of char-server: %s.\n", lan_char_ip);
