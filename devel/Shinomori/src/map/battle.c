...
struct Damage battle_calc_attack(int attack_type, struct block_list *bl,struct block_list *target, int skill_num,int skill_lv,int flag)
{
	...
	default:
		if(battle_config.error_log)
			ShowMessage("battle_calc_attack: unknwon attack type ! %d\n",attack_type);
		memset(&d,0,sizeof(struct Damage));
		break;
	}
	return d;
}
...
int battle_weapon_attack( struct block_list *src,struct block_list *target,unsigned long tick,int flag)
{
	...
	if(battle_check_target(src,target,BCT_ENEMY) > 0 &&
		...
		if(sc_data && sc_data[SC_AUTOSPELL].timer != -1 && rand()%100 < sc_data[SC_AUTOSPELL].val4) {
			int f=0;
			short skilllv = (short)sc_data[SC_AUTOSPELL].val3;
			int i = rand()%100;
...
printf("battle_weapon_attack negative skill trap 2\n");
		...
		if(t_sc_data && t_sc_data[SC_POISONREACT].timer != -1 && t_sc_data[SC_POISONREACT].val4 > 0)
		{	// poison react [Celest]
...
static const struct {
	...
	{ "max_parameter",               (int*)&battle_config.max_parameter			},
...
// eAthena additions
	...
	{ "maximum_level",               (int*)&battle_config.maximum_level	},	// [Valaris]
	...
	{ "pk_min_level",     (int*)&battle_config.pk_min_level}, // [celest]
...
int battle_set_value(char *w1, char *w2) {
	...
	for(i = 0; i < sizeof(battle_data) / (sizeof(battle_data[0])); i++)
		if (strcasecmp(w1, battle_data[i].str) == 0) {
			*((unsigned int *) battle_data[i].val) = config_switch(w2);
			return 1;
		}
	return 0;
}
